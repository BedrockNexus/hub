...
const NON_ALPHANUMERIC_SPLIT_PATTERN = /[^a-zA-Z0-9]+/
...
function getProcessedFilterValue(value: string | string[]) {
	if (Array.isArray(value)) {
		return value
	}
	if (NON_ALPHANUMERIC_PATTERN.test(value)) {
		return value.split(NON_ALPHANUMERIC_SPLIT_PATTERN).filter(Boolean)
	}
	return [value]
}
...
export function useDataTable<TData>(props: UseDataTableProps<TData>) {
	...
	const initialColumnFilters: ColumnFiltersState = useMemo(() => {
		...
		return Object.entries(filterValues).reduce<ColumnFiltersState>(
			(filters, [key, value]) => {
				if (value !== null) {
					const processedValue = getProcessedFilterValue(value)
